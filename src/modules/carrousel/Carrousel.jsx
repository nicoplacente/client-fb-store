...
export default function SpreenCarrousel() {
  ...
  return (
    <Swiper
      className="w-full mb-36 [mask-image:linear-gradient(to_right,transparent_0%,black_10%,black_90%,transparent_100%)]"
    ...
    >
      {items.map((item, i) => (
        <SwiperSlide
          key={i}
          className={`!flex !flex-col py-9 !items-center overflow-hidden !justify-between bg-[#17171799] sm:backdrop-blur-3xl rounded-2xl !w-[375px] !h-[485px]  text-white shadow-lg ${
            activeIndex === i ? "active-card" : ""
          }`}
        >
          <div className="!flex !flex-col gap-6 ">
            ...
            />
            <h3 className="font-bold text-3xl text-center font-mono text-balance">
              {item.title}
            </h3>
          </div>
