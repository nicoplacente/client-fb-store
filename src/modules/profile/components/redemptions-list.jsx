...
function RedemptionItem({ redemption, onAudioUpdated }) {
  const isCreditPurchase = redemption.redemptionType === "credit_purchase";
  const fallbackIcon = isCreditPurchase ? (
    <IconCoins size={34} />
  ) : (
    <IconShoppingBag size={34} />
  );
  const description = isCreditPurchase
    ? redemption.product.description || "Compra de pack de creditos"
    : redemption.product.description || "Canje de producto";
  const creditLabel = isCreditPurchase ? "créditos recibidos" : "créditos";

  return (
    <article className="grid gap-4 rounded-lg border border-white/10 bg-neutral-900/60 p-4 sm:grid-cols-[112px_1fr]">
      ...
      <div className="flex flex-col justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-4 text-sm text-neutral-400">
          ...
          <span className="inline-flex items-center gap-2 text-yellow-200">
            ...
            {redemption.cost.toLocaleString()} {creditLabel}
          </span>
          <span className="inline-flex items-center gap-2">
