...
function PredictionsHero({ activeCount, totalPool, availablePoints, onRefresh }) {
  const stats = [
    {
      label: "En vivo",
...
function PredictionCard({
  ...
  const bettingClosed = prediction.endsAt
    ...
    : false;
  const availableBetPoints = Math.max(0, Math.floor(Number(availablePoints || 0)));
        ...
        <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_19rem] lg:items-stretch">
          ...
          <div className="mt-3 rounded-xl border border-white/10 bg-neutral-900/45 p-3 text-xs leading-5 text-neutral-400">
            <p className="flex items-start gap-2">
              <IconInfoCircle className="mt-0.5 shrink-0 text-red-200" size={16} />
              {selectedOption
    ...
    <article className="relative overflow-hidden rounded-3xl border border-amber-300/18 bg-[linear-gradient(180deg,rgba(23,23,23,0.92),rgba(10,10,10,0.92))] p-4 shadow-2xl shadow-black/25 ring-1 ring-white/[0.03] sm:p-6">
      ...
      <div className="relative mt-5 flex flex-col gap-3 rounded-2xl border border-amber-300/35 bg-amber-400/10 px-4 py-3 text-sm text-amber-100 sm:flex-row sm:items-center sm:justify-between">
        ...
        <span className="text-xs font-semibold uppercase tracking-wide text-amber-200/80">
          {selectedOption ? `Tu opcion: ${selectedOption.label}` : "Sin apuesta registrada"}
        </span>
      </div>
    </article>
  );
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-3">
        ...
        {selected ? (
          <span className={`inline-flex items-center gap-1 rounded-full border px-3 py-1 text-[10px] font-black uppercase ${accent.softBorder} ${accent.text}`}>
...
function PredictionOption({ option, accent, selected, locked, onSelect }) {
  return (
    <button
      ...
      onClick={onSelect}
      disabled={locked}
      className={`group/option cursor-pointer rounded-2xl border bg-neutral-950/55 p-4 text-left transition hover:-translate-y-0.5 hover:bg-neutral-900/70 focus:outline-none focus:ring-2 focus:ring-red-300/35 disabled:cursor-not-allowed disabled:hover:translate-y-0 ${
...
function ResolvedOutcomePanel({ prediction, outcome }) {
  ...
  return (
    <div className={`mt-5 rounded-2xl border p-5 text-center shadow-inner shadow-black/15 ${panelClasses}`}>
      <p className="inline-flex items-center justify-center gap-2 text-xs font-black uppercase tracking-wide">
        {won ? <IconTrophy size={16} /> : lost ? <IconAlertTriangle size={16} /> : <IconInfoCircle size={16} />}
...
function HistoryFilterButton({ active, label, count, onClick }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className={`inline-flex min-h-11 cursor-pointer items-center gap-2 rounded-xl border px-4 py-2 text-xs font-black uppercase tracking-wide transition focus:outline-none focus:ring-2 focus:ring-red-300/40 ${
        active
    ...
    >
      {label} <span className="rounded-full bg-white/[0.06] px-2 py-0.5 text-neutral-400">{count}</span>
    </button>
  );
}
    ...
    <article className="grid gap-3 rounded-2xl border border-white/10 bg-neutral-950/70 px-4 py-3 shadow-xl shadow-black/10 transition hover:border-red-300/20 hover:bg-neutral-900/70 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center">
      <div className="min-w-0">
        ...
        <p className="mt-1 flex flex-wrap items-center gap-x-2 gap-y-1 text-xs font-semibold uppercase tracking-wide text-neutral-600">
          ...
          <span>{formatNumber(outcome.betPoints || prediction.totalPool)} pts</span>
          <span>{getRelativeTime(prediction.resolvedAt || prediction.endsAt)}</span>
        </p>
      </div>
      <span
